...
import csv
import numpy as np
path_input = '/Volumes/Cisco/Fall2021/Devops/Final-project/v2/data/'
...
Repo = df_input.Repo.values.tolist()
...
list_category = list(option_set)
for topic in set(manual_topic):
    print(topic)
    data_file = open(path + '/instructions/processed_instruction_evolution_{}.csv'.format(str(topic_dict.get(topic)).lower()), mode='w', newline='', encoding='utf-8')
    ...
    for i in range(len(Repo)):
        if manual_topic[i] == topic:
            ...
            for j in range(len(repos)):
                ...
                if Repo[i] == repos[j] and repo_version[j] and option[j] in instruct_dict:
