...
data_file = open(path + '/processed_instructions_selected.csv', mode='w', newline='', encoding='utf-8')
            ...
            if manual_topic[i] == topic:
                ...
                for j in range(len(repos)):
                    ...
                    if Repo[i] == repos[j]: # in selected_tag_list:
                        if option[j] in data_options_dict.keys():
                            if repo_version[j] in data_options_dict.get(option[j]).keys():
                                data_options_dict[option[j]][repo_version[j]] += option_count[j]
                            else:
                                data_options_dict[option[j]][repo_version[j]] = option_count[j]
                        else:
                            data_options_dict[option[j]] = {}
                            data_options_dict[option[j]][repo_version[j]] = option_count[j]
