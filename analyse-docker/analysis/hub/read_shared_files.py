...
def get_layers(data):
    ...
    data_file = open(path_output + '/RQ1b/processed_layers_shared2.csv', mode='w', newline='', encoding='utf-8')
        ...
        for doker_image in images_lists:
                    ...
                    for a in range(len(Image)):
                        if 'share' in str(file_path[a]).lower():
                            if Image[a] in shared_files_data.keys():
                                if RepoTag[a] in shared_files_data.get(Image[a]).keys():
                                    shared_files_data[Image[a]][RepoTag[a]] += 1
                                    memory_data[Image[a]][RepoTag[a]] += file_size[a]
                        ...
                        if Image[a] in total_files_data.keys():
                            if RepoTag[a] in total_files_data.get(Image[a]).keys():
                                total_files_data[Image[a]][RepoTag[a]] += 1
                                total_files_memory_data[Image[a]][RepoTag[a]] += file_size[a]
                            else:
                                total_files_data[Image[a]][RepoTag[a]] = 1
                                total_files_memory_data[Image[a]][RepoTag[a]] = file_size[a]
                        else:
                            total_files_data[Image[a]] = {}
                            total_files_data[Image[a]][RepoTag[a]] = 1

                            total_files_memory_data[Image[a]] = {}
                            total_files_memory_data[Image[a]][RepoTag[a]] = file_size[a]
            ...
            for key, val in shared_files_data.items():
                ...
                for key2, val2 in val.items():
                    total = total_files_data[key][key2]
                    data_writer.writerow([current_topic, repo, 'P{}'.format(dict_topic_repo.get(current_topic).index(repo)), key2,'Files shared', round(val2*100/total)])
                    data_writer.writerow(
                        [current_topic, repo, 'P{}'.format(dict_topic_repo.get(current_topic).index(repo)), key2,
                         'Memory',
                         round(memory_data[key][key2] * 100 / total_files_memory_data[key][key2])])
