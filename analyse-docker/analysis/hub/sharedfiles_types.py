...
def get_layers(data):
    ...
    for top in set(manual_topic):
        data_file = open(path_output + '/RQ1b/processed_{}.csv'.format(topic_dict.get(top).lower()), mode='w', newline='',
                         encoding='utf-8')
                    ...
                    for key, val in shared_files_data.items():
                        ...
                        for key2, val2 in sorted_d.items():
                            if check_isnumeric(key2) == False:
                                data_writer.writerow([current_topic, repo, 'P{}'.format(dict_topic_repo.get(current_topic).index(repo)), key2, round((val2*100/total),2), round((memory_files_data[key][key2]*100/total_memory_files_data.get(key)),4)])
                                index_count += 1
                                if index_count == 6:
                                    break
