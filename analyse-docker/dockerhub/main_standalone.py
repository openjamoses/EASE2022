...
class DockerImages:
    ...
    def get_info_images(self, image_list, extraction_version=1):

        #dt_string = datetime.now().strftime("%Y-%m-%d-%H-%M")
        data_file_stats = open(self.csv_path + '/Images_Info_stats_{}.csv'.format(extraction_version), mode='w', newline='',
                               encoding='utf-8')
        ...
        data_file_tags = open(self.csv_path + '/Images_tags_{}.csv'.format(extraction_version), mode='w', newline='',
                              encoding='utf-8')
        ...
        data_file_layers = open(self.csv_path + '/Images_layers_{}.csv'.format(extraction_version), mode='w', newline='',
                                encoding='utf-8')
        ...
        for doker_image in image_list:
            ...
            try:
                ...
                downloader = Downloader(root_path=path_download, layer_path=self.layers_path,
                                        run_version=extraction_version, image_name=image_name)
                ...
                downloader.download_manifest_groups(dict_image_tags)
...
list_images = ['0xbadb100d/computervision-recipes', 'automlbenchmark/autosklearn', 'mmdnn/mmdnn', 'optuna/optuna', 'yukkee/coremltools', 'mlflow/mlflow', 'seldon-core-operator']
print('Total images: ', len(list_images))
dockerImages.get_info_images(list_images, extraction_version=1)
