    ...
    def list_project_docker_files(directory):
        ...
        for (dirpath, dirnames, filenames) in os.walk(directory):
            ...
            for file in filenames:
                if 'dockerfile' in str(file).lower():
                    docker_files.append(os.path.join(dirpath, file))
            list_of_files.extend(docker_files)
        return list_of_files

    @staticmethod
    def list_project_docker_compose(directory):
        directory = os.path.abspath(directory)
        list_of_files = list()
        for (dirpath, dirnames, filenames) in os.walk(directory):
            docker_files = list()
            for file in filenames:
                if 'docker-compose' in str(file).lower():
                    docker_files.append(os.path.join(dirpath, file))
            list_of_files.extend(docker_files)
        return list_of_files

    @staticmethod
    def list_project_other_docker_relatedfiles(directory):
        directory = os.path.abspath(directory)
        list_of_otherfiles = list()
        for (dirpath, dirnames, filenames) in os.walk(directory):
            docker_files = list()
            for file in filenames:
                if 'docker' in str(file).lower() and not 'dockerfile' in str(file).lower():
                    docker_files.append(os.path.join(dirpath, file))
            list_of_otherfiles.extend(docker_files)
        return list_of_otherfiles
